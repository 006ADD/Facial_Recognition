...
public class ViewerPersonController extends javax.swing.JDialog {
    private KGradientPanel kGradientPanel1;
    int id;
    Connecor connect = new Connecor();
    ControllerDB exitDAO = new ControllerDB();
    
    /*public Viewer(String user) {
        initComponents();
    }*/
   public ViewerPersonController(java.awt.Frame parent, boolean model) throws SQLException {
    super(parent, model);
    initComponents(); // Вызов инициализации компонентов
    paint_table();
    exitDAO.filltable("SELECT * FROM Controller",table);

}
    
    public ViewerPersonController(String user) throws SQLException{
        initComponents();
        paint_table();
        exitDAO.filltable("SELECT * FROM Controller ",table);
    }
    ...
    private void txt_searchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_searchKeyReleased
        try {
            exitDAO.filltable("SELECT * FROM Controller WHERE person_id LIKE" + txt_search.getText() + 
                              "%' OR enterDate LIKE '%" + txt_search.getText() + 
                              "%' OR exitDate LIKE '%" + txt_search.getText() + 
                                "%' ORDER BY person_id'", table);
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    ViewerPersonController dialog = new ViewerPersonController(new javax.swing.JFrame(), true);
                    ...
                    Logger.getLogger(ViewerPersonController.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        });    
    }
